...
class Orchestrator(app_manager.RyuApp):
    ...
    def DM_update_bandwidth(self, dpid, port, rxBytes, txBytes):
        ...
        if port not in self.m_switchFlowStats[dpid]:
            ...
            if deltaTime == float(0.0):
                LOG_DEBUG("This scenario should not occur. Programming or System Error!!!")
                return
    ...
    def _packet_in_handler(self, ev):
        ...
        # First Packet of an SLA, installing respective flow rules
            ...
            # Install SLA based Flow Rules (also, if updated)
            if sla_object.compPathOfSLA[src_mac][dst_mac]['isInstalled'] == False:

                # Note Old Flow 
                oldPrioirty = sla_object.compPathOfSLA[src_mac][dst_mac]['priority']
                ...
                sla_object.compPathOfSLA[src_mac][dst_mac]['isInstalled'] = True
            ...
            # Tweak : aich Node Bottleneck
            global gNodeBottleneckHackCount
            if gNodeBottleneckHackCount == True:
                gNodeBottleneckHackCount = False
                LOG_DEBUG("Tweaked Bottleneck detected at Hypervisor(3).")
                self.handleNodeBottleneck(3)
    ...
    def retreivePathOfSLA(self, sla):
        ...
        for src_mac in sla.endUsersMac:
            for dst_mac in sla.endUsersMac:
                ...
                allPaths[src_mac][dst_mac]['isInstalled'] = False
                allPaths[src_mac][dst_mac]['priority']    = START_PRIORITY_VAL

                # Path of SLA Chain
                if True or self.debugFlag:
                    LOG_DEBUG("SLA Complete Path (Src %s to Dst %s) : %s"  % (src_dpid, dst_dpid, path))
    ...
    def removeOldSLAFlowRules(self, sla, src_mac, dst_mac, path):

        pdb.set_trace()

        LOG_DEBUG("-----------------------------------------------------------------------------------------")
        LOG_DEBUG("Handling Path : %s -> %s" % (src_mac, dst_mac))
        ...
        while slaIndex < len(path):
            ...
            LOG_DEBUG("-- Removed Datapath : %s | Match : vlan_tag-%s in_port-%s eth_src-%s eth_dst-%s | Output : out_port-%s" % (this_dpid, sla.vlanCommonTag | processed_vnf_cnt, in_port, src_mac, dst_mac, out_port))
        ...
        while True:
            ...
            if path[slaIndex][1] != 0:
                ...
                # Receiving Packet for first time
                    ...
                    match = parser.OFPMatch(in_port=in_port, eth_src=src_mac, eth_dst=dst_mac)
                    self.del_flow(datapath, priority, match)
                    LOG_DEBUG("-- Removed Datapath : %s | Match : in_port-%s eth_src-%s eth_dst-%s | SetFeild : eth_dst - %s, Output : out_port-%s" % (this_dpid, in_port, src_mac, dst_mac, vnf_mac, vnf_out_port))

                else:
                    # Check VLAN Tag
                    match = parser.OFPMatch(vlan_vid=(sla.vlanCommonTag | processed_vnf_cnt),in_port=in_port,eth_src=src_mac,eth_dst=dst_mac)
                    self.del_flow(datapath, priority, match)
                    LOG_DEBUG("-- Removed Datapath : %s | Match : vlan_vid-%s in_port-%s eth_src-%s eth_dst-%s | SetFeild : eth_src - %s, eth_dst - %s, Output : out_port-%s" % (this_dpid, sla.vlanCommonTag | processed_vnf_cnt, in_port, src_mac, dst_mac, self.m_ovs_mac[this_dpid], vnf_mac, vnf_out_port))
                ...
                if this_dpid == next_switch:
                    out_port     = ofproto.OFPP_IN_PORT
                    next_vnf_mac = sla.VNFsNetworkMac[processed_vnf_cnt - 1]

                    isMultipleVNFsAtSameDpid = True
                    LOG_DEBUG("-- Removed Datapath : %s | Match : in_port-%s eth_src-%s eth_dst-%s | SetFeild : eth_src - %s, eth_dst-%s Output : out_port-%s" % (this_dpid, vnf_out_port, vnf_mac, dst_mac, src_mac, next_vnf_mac, out_port))

                else:
                    # Add VLAN Tag for processing in next Switch(s)
                    out_port = self.m_graph['edges'][this_dpid][next_switch]
                    LOG_DEBUG("-- Removed Datapath : %s | Match : in_port-%s eth_src-%s eth_dst-%s | SetFeild : vlan_tag-%s, eth_src - %s, Output : out_port-%s" % (this_dpid, vnf_out_port, vnf_mac, dst_mac, sla.vlanCommonTag | processed_vnf_cnt, src_mac, out_port))
            ...
            if prev_switch == dst_dpid:
                ...
                match     = parser.OFPMatch(in_port=same_port,eth_src=last_vnf_mac,eth_dst=dst_mac)
                self.del_flow(datapath, priority, match)
                LOG_DEBUG("-- Removed Datapath : %s | Match : in_port-%s eth_src-%s eth_dst-%s | SetFeild : eth_src - %s, Output : out_port-%s" % (dst_dpid, same_port, last_vnf_mac, dst_mac, src_mac, ofproto.OFPP_IN_PORT))
            ...
            ## Case 3b. Last VNF Hypervisor and Egress Switch are not same
                ...
                while path[slaIndex][1] != -1:
                    ...
                    ofproto     = datapath.ofproto
                    next_switch = path[slaIndex + 1][0]
                    in_port     = last_vnf_in_port if slaIndex == 0 else self.m_graph['edges'][this_dpid][prev_switch]
                    out_port    = self.m_graph['edges'][this_dpid][next_switch]
                    out_port    = ofproto.OFPP_IN_PORT if out_port == in_port else out_port
                    ...
                    if this_dpid == last_vnf_dpid:
                        match   = parser.OFPMatch(in_port=in_port, eth_src=last_vnf_mac, eth_dst=dst_mac)
                        self.del_flow(datapath, priority, match)
                        LOG_DEBUG("-- Removed Datapath : %s | Match : in_port-%s eth_src-%s eth_dst-%s | SetFeild : vlan_tag-%s eth_src - %s, Output : out_port-%s" % (this_dpid, in_port, last_vnf_mac, dst_mac, sla.vlanCommonTag | processed_vnf_cnt, src_mac, out_port))
            ...
            if prev_switch != dst_dpid:
                ...
                match    = parser.OFPMatch(vlan_vid=(sla.vlanCommonTag | processed_vnf_cnt), in_port=in_port, eth_dst=dst_mac)
                self.del_flow(datapath, priority, match)
                LOG_DEBUG("-- Removed Datapath : %s | Match : vlan_tag-%s in_port-%s eth_dst-%s | Output : out_port-%s" % (dst_dpid, sla.vlanCommonTag | processed_vnf_cnt, in_port, dst_mac, out_port))
            ...
            if len(path) > 2 and path[0][0] != path[1][0]:
                ...
                LOG_DEBUG("-- Removed Datapath : %s | Match : in_port-%s eth_src-%s eth_dst-%s | Output : vlan_vid-%s out_port-%s" % (src_dpid, src_in_port, src_mac, dst_mac, sla.vlanCommonTag, out_port))
    ...
    def update_topology_hostAdd(self, ev):
        ...
        LOG_DEBUG("Host added @ dpid - %s : %s via port - %s" % (dpid, mac, port))
    ...
    def updateNewSLAFlowRules(self, sla, isForAllPairs, in_src_mac, in_dst_mac):
        ...
        for src_mac in srcUsersMac:
            for dst_mac in dstUsersMac:
                ...
                # Installation of the respective flow Rules will be done when End-points intiates communication
                # Already installed Flow Rules have to be overwritten
                # Reference : installSLAFlowRules(sla)
                if sla.compPathOfSLA[src_mac][dst_mac]['isInstalled'] == True:

                    # Use Old prioirty to delete previous set of Flow Rules
                    ...
                    while slaIndex < len(path):
                        ...
                        actions     = [parser.OFPActionOutput(out_port)]
                        self.add_flow(datapath, priority, match, actions)
                        LOG_DEBUG("New Datapath : %s | Match : vlan_tag-%s in_port-%s eth_src-%s eth_dst-%s | Output : out_port-%s" % (this_dpid, sla.vlanCommonTag | processed_vnf_cnt, in_port, src_mac, dst_mac, out_port))
                    ...
                    while True:
                        ...
                        if path[slaIndex][1] != 0:
                            ...
                            # Receiving Packet for first time
                                ...
                                self.add_flow(datapath, priority, match, actions)
                                LOG_DEBUG("Datapath : %s | Match : vlan_vid-%s in_port-%s eth_src-%s eth_dst-%s | SetFeild : eth_src - %s, eth_dst - %s, Output : out_port-%s" % (this_dpid, sla.vlanCommonTag | processed_vnf_cnt, in_port, src_mac, dst_mac, self.m_ovs_mac[this_dpid], vnf_mac, vnf_out_port))
                    ...
                    if len(path) > 2 and path[0][0] != path[1][0]:
                        ...
                        self.add_flow(datapath, priority, match, actions) # Flow-Mod
                        LOG_DEBUG("Datapath : %s | Match : in_port-%s eth_src-%s eth_dst-%s | Output : vlan_vid-%s out_port-%s" % (src_dpid, src_in_port, src_mac, dst_mac, sla.vlanCommonTag, out_port))
    ...
    def migrateVNF_RestOp(self, savedContext, new_dpid):
        ...
        # May also be updated when PacketIn for First Time
        for src_mac in sla.endUsersMac:
            for dst_mac in sla.endUsersMac:
                ...
                if sla.compPathOfSLA[src_mac][dst_mac]['prevPath'] != []:
                    ...
                    #pdb.set_trace()
                    self.removeOldSLAFlowRules(sla, src_mac, dst_mac, sla.compPathOfSLA[src_mac][dst_mac]['prevPath'])
                    sla.compPathOfSLA[src_mac][dst_mac]['prevPath'] = []
    ...
    def migrateVNF_VNFStartOp(self, dpid, new_dpid, vnfId):
        ...
        ## Step 1 : Start VNF at the Nbr Hypervisor with same configuration
        ...
        ## Step 2a :  Save current context

        # Sanity Check
        if newVNFInfo.macAddr in self.m_vnfOperationsOnStart:
            LOG_DEBUG("This scenario should not occur. Highly impossible!!!")
            return
    ...
    def handleNodeBottleneck(self, dpid):
        ...
        for vnfIndex in self.m_hypVNFStatus[dpid]:
            ...
            LOG_DEBUG("Hypervisor (%s) : VNF (c%s) considered for Migration Algorithm (Case 1)." % (dpid, vnfInfo.identifier))
            ...
            for pair in priorityScores:
                LOG_DEBUG("Moving VNF (c%s) from Current (%s) to Nbr (%s) Hypervisor : %s." % (vnfInfo.identifier, dpid, pair.val2, priorityScores[pair]))
            ...
            if maxPriorityScore == 0.0:
                LOG_DEBUG("This scenario should not occur (NOW). Programming Error!!!")
                self.handleNodeBottleneckCase2()
