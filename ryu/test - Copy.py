...
class Orchestrator(app_manager.RyuApp):
    ...
    def _slaInputThread(self):
        ...
        slaObject  = struct_SLA(self.m_SLAsCount, ['Firewall', 'Firewall'], [10, 10], 10, 100, ['00:00:00:00:00:11', '00:00:00:00:00:42'], [1,4])
    ...
    def sendForwadingVNFCommand(self, vnfInfo, forwardToIP, sockfd, isLastVNFToDest):
        ...
        if isLastVNFToDest == False:
            ...
            #msgs.append("docker exec %s iptables -t filter -A FORWARD -d %s -j ACCEPT" % (cont_name, forwardToIP))
            #msgs.append("docker exec %s iptables -t nat -A PREROUTING -j DNAT --to-destination %s" % (cont_name, forwardToIP))
        else:
            msgs.append("docker exec %s iptables -A FORWARD -i eth0 -o eth0 -j ACCEPT" % cont_name)
    ...
    def installVNFsAtHypervisors(self, sla, toBeInstalledVNFInfo):
        ...
        self.sendForwadingVNFCommand(vnfInfo, False, sockfd, True)
        return SUCCESS
    ...
    def migrateVNF_VNFStartOp(self, dpid, new_dpid, vnfId):
        ...
        if newVNFInfo.servChainIndex != len(sla.vnfInputList) - 1:
            ...
            # Forward to Next VNF in the Service Chain
            self.sendForwadingVNFCommand(newVNFInfo, nextVNFInfo.ipAddr, sockfd, False)

        else:
            # Forward from Last VNF to Destination Host
            self.sendForwadingVNFCommand(newVNFInfo, False, sockfd, True)
    ...
    def handleNodeBottleneck(self, dpid):
        ...
        for vnfIndex in self.m_hypVNFStatus[dpid]:
            ...
            for pair in priorityScores:
                LOG_INFO("Moving VNF (c%s) from Current (%s) to Nbr (%s) Hypervisor : %s." % (vnfInfo.identifier, dpid, pair.val2, priorityScores[pair]))
